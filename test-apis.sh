...
for i in {0..10}
  do
    ...
    printf "\n"
    curl "${api_base_url_predev}/self-referring?port=8081"
    curl "${api_base_url_dev}/self-referring?port=8082"
    curl "${api_base_url_staging}/self-referring?port=8083"
    curl "${api_base_url_prod}/self-referring?port=8084"
    printf "\n"
